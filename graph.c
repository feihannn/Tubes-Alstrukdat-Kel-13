...
addressGraph AlokasiGraph(infotypeListL X){
    addressGraph P;
    P = (addressGraph) malloc (sizeof(ElmtGraph));
    if (P != NULL) {
        InfoGraph(P) = X;
        NextGraph(P) = NULL;
        CreateEmptyListL(&Link(P));
    }
    return P;
}
...
void AddLink(Graph *G, infotypeListL n, infotypeListL p){
    /* KAMUS LOKAL */
...
void PrintLink(Graph G, infotypeListL X){
    /* KAMUS LOKAL */
    addressGraph P;
    ListL L;
...
boolean IsLinked(Graph G, infotypeListL n, infotypeListL p){
    /* KAMUS LOKAL */
    addressGraph addrGn;
...
addressGraph SearchGraph(Graph G, infotypeListL X){
    /* KAMUS LOKAL */
    addressGraph addrG;
